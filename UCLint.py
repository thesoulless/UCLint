...
from PyQt4.QtGui import QApplication,  QFileDialog,  QPlainTextEdit, QStatusBar, QFileSystemModel
...
class UCLint():
    ...
    def lint(self, fileName):        
        try:
            ...
            for line in lines:            
                #if (self.stip_comments(line)):
                #    contents += self.stip_comments(line)            
                line = line.strip()
            ...
            self.ui.statusBar.showMessage("Unexpected error:", sys.exc_info()[0])
            raise
        else:
            f.close()
